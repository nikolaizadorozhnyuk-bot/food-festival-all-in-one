...
CONFIG = {
    ...
    "TG_TOKEN": "8275141603:AAGTvEF59ZOaD0rGsXHkitiWOA6TX-wTpRU", # Твій робочий токен
...
st.markdown("""
    ...
    div.stButton > button:disabled { background-color: #e0e0e0 !important; color: #888 !important; }
...
def show_catalog(u):
    ...
    # Визначаємо колонки автоматично
    ...
    if "Дешевші" in sort_opt:
        f_df['p_val'] = f_df[p_col_sys].apply(lambda x: float(str(x).replace(',','.')) if x else 0)
        f_df = f_df.sort_values('p_val')
    elif "Дорожчі" in sort_opt:
        f_df['p_val'] = f_df[p_col_sys].apply(lambda x: float(str(x).replace(',','.')) if x else 0)
        f_df = f_df.sort_values('p_val', ascending=False)
    ...
    for i in range(0, len(items), 3):
        ...
        for j, item in enumerate(items[i:i+3]):
            with cols_ui[j]:
                is_promo = "Акція" in item.get('Категорія', '') or "ТОП" in item.get(name_col, '')
                ...
                if is_promo: st.warning("🔥 ТОВАР ДНЯ")
                st.subheader(item[name_col])
                
                with st.expander("📖 Опис продукту"):
                    st.write(item.get('Опис', 'Смачно, корисно та якісно.'))
...
def show_cart(u):
    ...
    if now < cutoff:
        st.markdown(f"<div class='status-alert' style='background: #E8F5E9; color: #2E7D32;'>🚀 Встигаєте на доставку СЬОГОДНІ!</div>", unsafe_allow_html=True)
    else:
        st.markdown(f"<div class='status-alert' style='background: #FAFAFA; color: #757575;'>📅 Доставка буде на ЗАВТРА.</div>", unsafe_allow_html=True)
    ...
    if total < CONFIG["MIN_ORDER"]:
        ...
        st.progress(total / CONFIG["MIN_ORDER"])
    else: st.success(f"✅ Мінімалку набрано! Сума: {total:g} ₴")
    ...
    st.info(f"🏢 Постачальник: **{active_fop}**")
    
    c1, c2 = st.columns(2)
    client_fop = c1.text_input("📝 Ваш ФОП / Назва закладу:", value=last_fop)
    ...
    if st.button("🚀 ПІДТВЕРДИТИ ЗАМОВЛЕННЯ", use_container_width=True, disabled=(total < CONFIG["MIN_ORDER"])):
        ...
        msg = (
            ...
            f"🏢 Наш ФОП: <b>{active_fop}</b>\n"
            f"━━━━━━━━━━━━━━━━━━━━\n"
            f"👤 Клієнт: {u['Назва']}\n"
            f"📝 ФОП Клієнта: {client_fop}\n"
            f"📍 Адреса: {addr}\n"
            f"📅 Дата: {d_date.strftime('%d.%m.%Y')}\n"
            f"💰 <b>СУМА: {total:g} ₴</b>\n"
            f"🛒 Товари: {items_txt}\n"
            f"💬 {comm}"
        )
        ...
        requests.post(CONFIG["SCRIPT_URL"], json={
            ...
            "fop_client": client_fop, "fop_our": active_fop, "date": d_date.strftime('%d.%m.%Y')
        })
        
        st.session_state.cart = {}
        st.success("✅ Замовлення надіслано успішно!")
        st.balloons()
...
def main():
    ...
    if not st.session_state.logged_in:
        ...
        choice = st.sidebar.selectbox("Меню", ["🍽️ Меню", "🛒 Кошик", "📜 Історія", "📰 Новини"])
